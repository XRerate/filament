...
FView::FView(FEngine& engine)
    ...
    auto& descriptorSet = mColorPassDescriptorSet.getDescriptorSet();
    descriptorSet.setBuffer(+PerViewBindingPoints::LIGHTS,
            mLightUbh, 0, sizeof(LightsUib));
    descriptorSet.setBuffer(+PerViewBindingPoints::RECORD_BUFFER,
            mFroxelizer.getRecordBuffer(), 0, sizeof(FroxelRecordUib));
    descriptorSet.setBuffer(+PerViewBindingPoints::FROXEL_BUFFER,
            mFroxelizer.getFroxelBuffer(), 0, sizeof(FroxelsUib));
}
...
void FView::prepare(FEngine& engine, DriverApi& driver, RootArenaScope& rootArenaScope,
    ...
    { // this must happen after mRenderableUbh is created/updated
        ...
        for (uint32_t const i : merged) {
            ...
            // FIXME: currently morphing always use the first target only
            ...
            if (UTILS_UNLIKELY(skinning.handle || morphing.handle || instance.handle)) {
                ...
                if (UTILS_UNLIKELY(skinning.handle || morphing.handle)) {
                    ...
                    descriptorSet.setSampler(+PerRenderableBindingPoints::MORPH_TARGET_POSITIONS,
                            morphing.targets[0].buffer->mPbHandle, {});
